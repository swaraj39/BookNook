...
app.get(
  "/api/borrow-requests",
  authenticate,
  AppController.myRequests
);

app.post(
  "/api/borrow-requests",
  authenticate,
  AppController.requestBook
);
...
app.get(
  "/api/loans/borrowed",
  authenticate,
  AppController.borrowed
);

app.get(
  "/api/loans/history",
  authenticate,
  AppController.loanHistory
);

app.post(
  "/api/loans/:id/return",
  authenticate,
  AppController.returnBook
);
...
app.get("/api/quote/today", async (req, res) => {
  try {
    ...
    logger.error(`Quote API Error: ${error.message}`);

    res.status(500).json({
      message: "Unable to fetch today's quote.",
    });
  }
});
...
app.listen(port, () => {
  logger.info(
    `BookNook API started successfully on http://localhost:${port}`
  );
});