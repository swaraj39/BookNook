        ...
        {page.content.map((row) => (
          <tr key={row.id}>
            ...
            <td><div className="row-actions">
              {(row.owner.id === me.id || isAdmin) && row.status === "pending" && <><button className="btn approve" onClick={() => approve(row.id)}>Approve</button><button className="btn danger" onClick={() => reject(row.id)}>Reject</button></>}
